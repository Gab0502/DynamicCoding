...
def cresce(lista):
    ...
    lista.append(lista[ultimo_indice]+1)
    return lista
...
def cria_baralho():
    ...
    baralho=[]
    for i in naipes:
        baralho_atual = cria_naipe(i)
        for j in baralho_atual:
            baralho.append(j)
    return  baralho
...
def  liquidacao(precos,descontos):
    ...
    for i in range(len(precos)):
        preco_desc = precos[i] - descontos[i]
        precos_finais.append(preco_desc)
